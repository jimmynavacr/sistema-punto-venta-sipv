...
namespace SIPV.Windows.Transacciones
{
    ...
    public partial class IU_AJUSTE : BaseCode.frmBaseMant_Grid_DataObj
    {
        #region Constructores

        public IU_AJUSTE(BaseCode.DB vDB, Form Parent)
            :
            base(vDB, Parent, new SIPV.Datos.MOVIMIENTO_ARTICULO(vDB))
        {
            InitializeComponent();
        ...
        {
            this.SqlQueryMant = "SELECT MOVIMIENTO ,ARTICULO FROM MOVIMIENTO_ARTICULO";
        ...
        {
            TextCampoLlave.Text = ((SIPV.Datos.MOVIMIENTO_ARTICULO)TablaBase).Movimiento;
        }
        public override void CargarObjsDeDatosDesdeObjsDeInterfaces()
        {

            ((SIPV.Datos.MOVIMIENTO_ARTICULO)TablaBase).Movimiento = TextCampoLlave.Text;
        }
        private void IU_TIPO_ENTIDAD_AntesDatoEnviado(object sender, EventArgs e)
        {
            ((SIPV.Datos.MOVIMIENTO_ARTICULO)TablaBase).Movimiento = DataGrid.DataGrid.Rows[DataGrid.DataGrid.CurrentCell.RowIndex].Cells[0].Value.ToString(); ;
        }
    }
}
